...
{   
    public function relatedproductlistincart(Request $request){
        $userid = $request->userid;
        $related = array();
         $cartdetail = DB::table("cart")->select("category_id")->where("customer_id",$userid)->groupby("category_id")->pluck('category_id');
           $relatedpr = Product::whereIN("category_id",$cartdetail)->orderby("id","desc")->limit(5)->get();
          ...
          return response()->json(["status"=>true,"code"=>100,"msg"=>"Successfully Show","data"=>$related]);
    }
        ...
        public function increasecartqty(Request $request){ 
         ...
         if($qty==0){
          ...
          $unitprice = $attribute->unitprice;
           ...
           $tae = ($totalprice * $attribute->tax)/100;
             DB::table("cartattribute")->where("id",$cartid)->update(["qty"=>$qty,"totalprice"=>$totalprice,"taxamount"=>$tae]);
         }
         return response()->json(["status"=>true,"code"=>100,"msg"=>"Successfully Show","grandtotal"=>$totalprice]);
     }
    ...
    public function usercart(Request $request){ 
        ...
        //   $cartdetail = DB::table("cart")->select("category_id","id")->where("customer_id",$userid)->groupby("category_id","id")->get();
           $cartdetail = DB::table("cart")->select("category_id")->where("customer_id",$userid)->groupby("category_id")->get();
          ...
          foreach($cartdetail as $crtdt){
              ...
              $rt[$categoryname]=array();$proarry=array();
                ...
                foreach($cartprod as $prod){ 
                    ...
                    $catary["productimage"]=$pnme->image;//echo '</br>'.$prod->id;
                    // $cartattribut = DB::table("cartattribute")->where("cart_id",$crtdt->id)->get();
                    $cartattribut = DB::table("cartattribute")->where("cart_id",$prod->id)->get();
          ...
          $combinecoupon = $cpnamt.'+'.$couponbyadmin;
                        ////////agr cart me addressid h to cgst sgst igst calculate honge ni to cgst r sgst jayenge
                           $cgst = $tax/2;
                            $sgst = $tax-$cgst;
                            $igst = 0;
                            $taxamount = ($granttotal*($cgst+$sgst))/100;
    ...
    public function deletecart(Request $request){ 
        $cartid = $request->get('cartid'); $granttotal=0;$cnt=0;
         ...
         return response()->json(["status"=>true,"code"=>100,"msg"=>"Deleted Successfully"]);
     }
    ...
    public function addtowishlist(Request $request){ 
        ...
        $save = DB::table("wishlist")->insert(
         ...
         return response()->json(["status"=>true,"code"=>100,"msg"=>"Added to wishlist","counter"=>$cnt,"data"=>$wishlistdetail]);
         }else{
        ...
        $userid = $request->get('userid'); $granttotal=0;$cnt=0;$wishlist=array();
          $wishlistdetail = DB::table("wishlist")->where("userid",$userid)->get();
             ...
             $prod = Product::where("id",$crtdt->productid)->first();
                 $dt["productid"]=$prod->id;
                 $dt["brandid"]=$prod->brand_id; 
                 ...
                 $proarrti = ProductAttributes::where("product_id",$prod->id)->orderby('id','asc')->get();
                 $dt['attribute']=$proarrti;
                //  $rlt['color']= Helper::getcolorsnamebyid($prod->color);
    ...
    public function deletewishlist(Request $request){ 
        ...
        $wishlistid = $request->get('wishlistid'); $granttotal=0;$cnt=0;
         ...
         return response()->json(["status"=>true,"code"=>100,"msg"=>"Wishlist Deleted Successfully","counter"=>$cnt,"data"=>$wishlistdetail]);
     }
}

    ...
    white-space: normal;
                                  ...
                                  <th>S.no</th>
                                        ...
                                        @foreach($list as $key=>$lt)
                                         <tr>
                                             <td>{{$key+1}}</td>
                                              <td><img src="{{ env('imagepath').'/'.$lt->image }}" width="100px"/>
                                      <form action="{{ route('seller.brand.update-image') }}" method="POST" enctype="multipart/form-data" class="mt-1">
                @csrf
                @method('PUT')
                <input type="hidden" name="old_image_name" value="{{ $lt->image }}">
                <input type="hidden" name="mid" value="{{ $lt->mid }}">
                <label for="file-{{ $key }}" style="cursor: pointer; color: #007bff; font-size: 12px; display: block;">
                    <i class="fa fa-camera"></i> Change Image
                </label>
                <input type="file" name="brand_image" id="file-{{ $key }}" accept="image/*" style="display: none;" onchange="this.form.submit()">
            </form></td>
                                             <td>{{$lt->name}}</td>
                                             ...
                                             <td  class="desc-col">
                                             @php
    $categories = explode(',', $lt->categories);

    $categoryNames = array_map(function($id) {
        return Helper::getcatname(trim($id));
    }, $categories);
@endphp
{{ implode(', ', $categoryNames) }}</td>
